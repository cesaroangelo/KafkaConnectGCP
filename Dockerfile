...
ADD start-kafkaConnect.sh /usr/bin/start-kafkaConnect.sh

ADD libs/*.jar /opt/kafka/libs/
ADD config/*.template /opt/kafka/config/

RUN chmod a+x /usr/bin/start-kafkaConnect.sh
WORKDIR $KAFKA_HOME
# Use "exec" form so that it runs as PID 1 (useful for graceful shutdown)
CMD ["start-kafkaConnect.sh"]
